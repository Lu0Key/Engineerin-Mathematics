...
发现 $G_1,\;G_2,\;G_3$ 相互独立, 因此可知 $\lambda_1,\;\lambda_2,\;\lambda_3$ \mybox{都为实数}, 且特征值的范围为

\mybox{$0\leqslant\lambda_1\leqslant2$, $3\leqslant\lambda_2\leqslant 7$, $8\leqslant \lambda_3 \leqslant 10$}.
...
\begin{enumerate}[(1)]
    \item $\varphi(x)=1+x^{-2}$, $\varphi'(x)=(1+x^2)=-2x^{-3}$, $|\varphi'(1.5)|=|-2\times 1.5^{-3}|\approx 1.78>1$, 该算式在 1.5 附近\mybox{不收敛}.
    \item $\varphi(x)=(1+x^2)^{\frac{1}{3}}$, $\varphi'(x)=\dfrac{2x}{3}(1+x^2)^{-\frac{2}{3}}$, $|\varphi'(1.5)|\approx 0.456 <1$, 该算式在 1.5 附近\mybox{收敛}.
    \item $\varphi(x)=(x-1)^{-\frac{1}{2}}$, $\varphi'(x)=-\dfrac{1}{2}(x-1)^{-\frac{3}{2}}$, $|\varphi'(1.5)|\approx 1.414 > 1$, 该算式在 1.5 附近\mybox{收敛}.
\end{enumerate}
