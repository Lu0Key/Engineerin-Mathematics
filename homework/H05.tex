...
\textbf{Sol}:
总体均值: $\alpha_1=E(x)$, 用矩估计法估计得到\mymathbox{\hat{\alpha}_1=\bar{x}=1.2}.\par
总体方差: $\alpha_2=M_k'(x)$, 用矩估计法得到 \mymathbox{\hat{\alpha}_2=S_x=0.488}.\par
利用总体均值估计参数: 由于
...
$$
则有 $\beta=2\alpha_1$。
所以 $\beta$ 矩估计值为 \mymathbox{\hat{\beta}_1=2\bar{x}=2.4}.
...
\mytextbox{从而得证上述式子为 $E(x)$ 的无偏估计量}. 其均方误差
...
均方误差 $r(t_1)=\dfrac{9}{25}S(X)$, $r(t_2)=\dfrac{7}{18}S(X)$, $r(t_3)=\dfrac{94}{196}S(X)$, 因此\mytextbox{无偏估计量 $t_1$ 方差最小}.
